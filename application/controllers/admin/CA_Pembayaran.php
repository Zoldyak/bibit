...
class CA_Pembayaran extends CI_Controller{
  ...
  function index()
  {
    ...
    $idkat=1;
      $listdata= $this->MA_Carabelanja->list_pembayaran()->result_array();

    $data=array(
          'halaman' => 'pembayaran/list.php',
          'daftar'  => $listdata
    );
  ...
  function tambah(){
    ...
    if($valid->run() != false){
      if (!empty($i->post('aksi'))) {
        ...
        redirect(base_url('admin/CA_Pembayaran'));
      }
    }
      $load=$this->load;
      $data=array(
            'halaman' => 'pembayaran/form.php',
            'jenis_form'=> 'tambah'
